...
class Shooter_Camera( wpilib.command.Subsystem ):
	...
	def __init__( self, robot ):
		...
		self.camera_mode = const.SHOOTER_CAMERA_GRIP


	def set_camera_mode( self, mode ):
		self.camera_mode = mode		

		if self.camera_mode == const.SHOOTER_CAMERA_STREAM:
			self.robot.nt_vision.putBoolean( "Shooter Camera Stream", True ) 
		else:
			self.robot.nt_vision.putBoolean( "Shooter Camera Stream", False ) 


	def toggle_camera_mode( self ):
		if self.camera_mode == const.SHOOTER_CAMERA_GRIP:
			self.set_camera_mode( const.SHOOTER_CAMERA_STREAM )
		else:
			self.set_camera_mode( const.SHOOTER_CAMERA_GRIP )
	...
	def get_target_values( self ):
		...
		for key in keys:
			try:
				values[ key ] = self.robot.nt_grip_boiler.getNumberArray( key )
			except:
	...
	def calculate_shooter_angle( self ):
		target_values = self.robot.shooter_camera.get_target_values( )
		print( target_values )
	...
	def log( self ):
		...
		wpilib.SmartDashboard.putBoolean( 'Shooter Targets Visible', targets_visible )
